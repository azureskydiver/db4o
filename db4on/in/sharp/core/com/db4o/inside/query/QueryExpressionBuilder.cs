...
namespace com.db4o.inside.query
{
	...
	{	
		public UnsupportedPredicateException(string reason) : base(reason)
		{	
		}
	}
	...
	{	
		...
		{
			if (method == null) throw new ArgumentNullException("method");
			if (1 != method.Parameters.Count) UnsupportedPredicate("A predicate must take a single argument.");
			if (0 != method.Body.ExceptionHandlers.Count) UnsupportedPredicate("A predicate can not contain exception handlers.");
			if (method.DeclaringType.Module.Import(typeof(bool)) != method.ReturnType.ReturnType) UnsupportedPredicate("A predicate must have a boolean return type.");
			...
			while (block != null)
			{
				switch (block.ActionType)
				{
					...
					case ActionType.Assign:
					{
						...
						{
							if (variables.Contains(variable.Variable.Index)) UnsupportedExpression(assign.Expression);
