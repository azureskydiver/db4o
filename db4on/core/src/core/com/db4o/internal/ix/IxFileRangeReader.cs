namespace com.db4o.@internal.ix
{
	...
	internal class IxFileRangeReader
	{
		...
		private readonly com.db4o.@internal.ix.Indexable4 _handler;
		...
		private readonly com.db4o.@internal.Buffer _reader;

		internal readonly int _slotLength;

		internal readonly int _linkLegth;

		internal IxFileRangeReader(com.db4o.@internal.ix.Indexable4 handler)
		{
			_handler = handler;
			_linkLegth = handler.LinkLength();
			_slotLength = _linkLegth + com.db4o.@internal.Const4.INT_LENGTH;
			_reader = new com.db4o.@internal.Buffer(_slotLength);
		}

		internal virtual com.db4o.foundation.Tree Add(com.db4o.@internal.ix.IxFileRange fileRange
			, com.db4o.foundation.Tree newTree)
		{
			SetFileRange(fileRange);
			com.db4o.@internal.LocalObjectContainer yf = fileRange.Stream();
			com.db4o.@internal.Transaction trans = fileRange.Trans();
			while (true)
			{
				...
				if (cmp == 0)
				{
					...
					cmp = parentID - ((com.db4o.@internal.ix.IxPatch)newTree)._parentID;
				}
				if (cmp > 0)
				{
					_upper = _cursor - 1;
				...
				else
				{
					...
					{
						if (newTree is com.db4o.@internal.ix.IxRemove)
						{
							if (_cursor == 0)
							{
		...
		internal virtual int Compare(com.db4o.@internal.ix.IxFileRange fileRange, int[] matches
			)
		{
			SetFileRange(fileRange);
			com.db4o.@internal.LocalObjectContainer yf = fileRange.Stream();
		...
		private int Compare(com.db4o.@internal.Transaction trans)
		{
			return _handler.CompareTo(_handler.ComparableObject(trans, _handler.ReadIndexEntry
				(_reader)));
		}

		private com.db4o.foundation.Tree Insert(com.db4o.@internal.ix.IxFileRange fileRange
			, com.db4o.foundation.Tree a_new, int a_cursor, int a_cmp)
		{
			...
			com.db4o.@internal.ix.IxFileRange ifr = new com.db4o.@internal.ix.IxFileRange(fileRange
		...
		private void SetFileRange(com.db4o.@internal.ix.IxFileRange a_fr)
		{
			_lower = 0;
