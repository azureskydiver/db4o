...
Namespace com.db4odoc.f1.semaphores
    ...
    Public Class ConnectedUser
        ...
        Public Shared Sub Configure()
            Dim objectClass As ObjectClass = Db4o.Configure().ObjectClass(GetType(ConnectedUser))
        ...
        Public Shared Sub Login(ByVal client As ObjectContainer, ByVal userName As String, ByVal ipAddress As String)
        ...
        Public Shared Function ConnectedUsers(ByVal server As ObjectServer) As IList
            Dim serverObjectContainer As ExtObjectContainer = server.Ext().ObjectContainer().Ext()
            If serverObjectContainer.SetSemaphore(SEMAPHORE_LOCK_ACCESS, TIMEOUT) Then
                Throw New Exception("Timeout Trying to get access to ConnectedUser lock")
            End If
            Dim list As IList = New ArrayList()
            Dim q As Query = serverObjectContainer.Query()
            q.Constrain(GetType(ConnectedUser))
