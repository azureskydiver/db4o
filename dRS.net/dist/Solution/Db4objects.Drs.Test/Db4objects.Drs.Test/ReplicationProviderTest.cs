namespace Db4objects.Drs.Test
{
	public class ReplicationProviderTest : Db4objects.Drs.Test.DrsTestCase
	{
		...
		private void TstObjectsChangedSinceLastReplication()
		{
			...
			System.Collections.IEnumerator cars = A().Provider().ObjectsChangedSinceLastReplication
				(typeof(Db4objects.Drs.Test.Car)).GetEnumerator();
			Db4oUnit.Assert.IsTrue(cars.MoveNext());
			Db4oUnit.Assert.AreEqual(((Db4objects.Drs.Test.Car)cars.Current).GetModel(), "Volvo"
				);
			Db4oUnit.Assert.IsFalse(cars.MoveNext());
			CommitReplication();
			StartReplication();
			Db4oUnit.Assert.IsFalse(A().Provider().ObjectsChangedSinceLastReplication().GetEnumerator
				().MoveNext());
			CommitReplication();
			Db4objects.Drs.Test.Pilot pilot = (Db4objects.Drs.Test.Pilot)A().Provider().GetStoredObjects
				(typeof(Db4objects.Drs.Test.Pilot)).GetEnumerator().Current;
			pilot._name = "Terry Jones";
			Db4objects.Drs.Test.Car car = (Db4objects.Drs.Test.Car)A().Provider().GetStoredObjects
				(typeof(Db4objects.Drs.Test.Car)).GetEnumerator().Current;
			...
			Db4oUnit.Assert.AreEqual(A().Provider().ObjectsChangedSinceLastReplication().Count
				, 2);
			pilots = A().Provider().ObjectsChangedSinceLastReplication(typeof(Db4objects.Drs.Test.Pilot)
				).GetEnumerator();
			Db4oUnit.Assert.AreEqual(((Db4objects.Drs.Test.Pilot)pilots.Current)._name, "Terry Jones"
				);
			Db4oUnit.Assert.IsFalse(pilots.MoveNext());
			cars = A().Provider().ObjectsChangedSinceLastReplication(typeof(Db4objects.Drs.Test.Car)
				).GetEnumerator();
			Db4oUnit.Assert.AreEqual(((Db4objects.Drs.Test.Car)cars.Current).GetModel(), "McLaren"
				);
			Db4oUnit.Assert.IsFalse(cars.MoveNext());
			CommitReplication();
