...
public class OldPartitionFieldNode implements IModelNode {
    ...
    public OldPartitionFieldNode(IModelNode[] toPartition, PartitionSpec partitionSpec, int depth, int position) {
    ...
    public IModelNode[] children() {
        if (_partitionSpec.hasPartitionChildren(_depth)) {
            ...
            for (int child=0; child < numChildren; ++child) {
                results[child] = new OldPartitionFieldNode(_toPartition, _partitionSpec, _depth+1, newNodePosition);
                ++newNodePosition;
            }
            return results;
        } else {
