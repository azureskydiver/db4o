...
public class LoggingArray implements ReflectArray {
	...
	public int flatten(Object a_shaped, int[] a_dimensions,
		...
		if (a_currentDimension == (a_dimensions.length - 1)) {
			...
			for (int i = 0; i < a_dimensions[a_currentDimension]; i++) {
				a_flatElement = flatten(getNoExceptions(a_shaped, i),
						a_dimensions, a_currentDimension + 1, a_flat,
						a_flatElement);
			}
		}
	...
	public Object newInstance(ReflectClass componentType, int length) {
		return Array.newInstance(
				JdkReflector.toNative(componentType), length);
	}

	public Object newInstance(ReflectClass componentType,
			int[] dimensions) {
		return Array.newInstance(
				JdkReflector.toNative(componentType), dimensions);
	}
	...
	public int shape(Object[] a_flat, int a_flatElement,
			Object a_shaped, int[] a_dimensions,
		...
		if (a_currentDimension == (a_dimensions.length - 1)) {
			...
			for (int i = 0; i < a_dimensions[a_currentDimension]; i++) {
				a_flatElement = shape(a_flat, a_flatElement, get(
						a_shaped, i), a_dimensions,
