namespace com.db4o.drs.inside.traversal
{
	public class GenericTraverser : com.db4o.drs.inside.traversal.Traverser
	{
		...
		public virtual void TraverseGraph(object _object, com.db4o.drs.inside.traversal.Visitor
			 visitor)
		{
			QueueUpForTraversing(_object);
			while (true)
			{
		...
		protected virtual void TraverseObject(object _object, com.db4o.drs.inside.traversal.Visitor
			 visitor)
		{
			if (!visitor.Visit(_object))
			{
				return;
			}
			com.db4o.reflect.ReflectClass claxx = _reflector.ForObject(_object);
			TraverseFields(_object, claxx);
		}

		protected virtual void TraverseFields(object _object, com.db4o.reflect.ReflectClass
			 claxx)
		{
			...
			TraverseFields(_object, superclass);
		}
		...
		protected virtual void QueueUpForTraversing(object _object)
		{
			if (_object == null)
			{
				return;
			...
			{
				TraverseCollection(_object);
				return;
			}
			if (claxx.IsArray())
			{
				TraverseArray(_object);
				return;
			}
			QueueAdd(_object);
		}

		protected virtual void QueueAdd(object _object)
		{
			_queue.Add(_object);
		}
