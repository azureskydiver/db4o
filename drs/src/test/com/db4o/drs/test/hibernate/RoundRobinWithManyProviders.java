...
public class RoundRobinWithManyProviders extends DrsTestCase {
	Configuration providerC;
	//HibernateReplicationProvider c ;
	public void test() {
		providerC = HibernateUtil.createNewDbConfig();
		providerC.addClass(Pilot.class);
		//HibernateReplicationProvider c = new HibernateReplicationProviderImpl(providerC);
	...
	private void replicateBC() {		
		HibernateReplicationProvider c = new HibernateReplicationProviderImpl(providerC);
		ReplicationSession sess = Replication.begin(b().provider(), c);
		
		ObjectSet changed = sess.providerA().objectsChangedSinceLastReplication();
	...
	private void replicateCA() {
		...
		ReplicationEventListener resolver =new ReplicationEventListener(){
			...
			public void onReplicate(ReplicationEvent e) {
				if (e.isConflict()) {
					//System.out.println("Conflict because C and A are never replicated.So they don't have a replication record. This makes both providers think the Pilot object was modified. ");
					e.overrideWith(e.stateInProviderA());
				}	
			}
			
		};
		ReplicationSession sess = Replication.begin( c, a().provider(),resolver);
		
		ObjectSet changed2 = sess.providerA().objectsChangedSinceLastReplication();
