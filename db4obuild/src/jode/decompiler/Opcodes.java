...
public abstract class Opcodes implements jode.bytecode.Opcodes
{
    ...
    private static StructuredBlock createSwitch
	(MethodAnalyzer methodanalyzer, Instruction instruction, int[] is,
    ...
    public static StructuredBlock readOpcode
	(Instruction instruction, MethodAnalyzer methodanalyzer)
	...
	case 152:
	    return createNormal(methodanalyzer, instruction,
				new CompareToIntOperator((types[4]
							  [(i - 145) / 2]),
							 (i == 150
							  || i == 152)));
	case 153:
	...
	case 171: {
	    ...
	    for (int i_6_ = 0; i_6_ < flowblocks.length; i_6_++)
		flowblocks[i_6_]
		    = (FlowBlock) instruction.getSuccs()[i_6_].getTmpInfo();
	    flowblocks[is.length]
		= (FlowBlock) instruction.getSuccs()[is.length].getTmpInfo();
	    return createSwitch(methodanalyzer, instruction, is, flowblocks);
	}
	case 172:
	case 173:
	case 174:
	...
	case 180: {
	    ...
	    return createNormal(methodanalyzer, instruction,
				new GetFieldOperator(methodanalyzer, i == 178,
						     reference));
	}
	case 179:
	case 181: {
	    ...
	    return (createNormal
		    (methodanalyzer, instruction,
		     new StoreInstruction(new PutFieldOperator(methodanalyzer,
							       i == 179,
							       reference))));
	}
	case 182:
	...
	case 185: {
	    Reference reference = instruction.getReference();
	    int i_7_ = (reference.getName().equals("<init>") ? 3 : i == 184 ? 2
			: i == 183 ? 1 : 0);
	    StructuredBlock structuredblock
