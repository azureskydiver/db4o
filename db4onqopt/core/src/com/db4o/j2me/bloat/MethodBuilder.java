...
public class MethodBuilder {
	...
	public MethodBuilder(
			...
			Class[] exceptions) {
		_context=context;
		_editor=new MethodEditor(classEditor,modifiers,type,name,params,exceptions);
		_labels=new HashMap();
	...
	public void invoke(int mode,Class parent,String name,Class[] params,Class ret) {
		invoke(mode,_context.getType(parent),name,params,ret);
	}

	public void invoke(int mode,Type parent,String name,Class[] params,Class ret) {
		_editor.addInstruction(mode, _context.methodRef(parent,name,params,ret));
	}
	...
	public void addTryCatch(int from,int to,int handler,Class thrown) {
		_editor.addTryCatch(new TryCatch(forceLabel(from),forceLabel(to),forceLabel(handler),_context.getType(thrown)));
	}
	
	public void getstatic(Type parent,Class type,String name) {
		_editor.addInstruction(Opcode.opc_getstatic,_context.fieldRef(parent,_context.getType(type),name));
	}

	public void getstatic(Type parent,Type type,String name) {
		_editor.addInstruction(Opcode.opc_getstatic,_context.fieldRef(parent,type,name));
	}

	public void putstatic(Type parent,Class type,String name) {
		_editor.addInstruction(Opcode.opc_putstatic,_context.fieldRef(parent,_context.getType(type),name));
	}
	...
	public static void createLoadClassConstMethod(Enhancer context,ClassEditor ce) {
		MethodBuilder bld=new MethodBuilder(context,ce, Modifiers.PROTECTED|Modifiers.STATIC, Class.class, "db4o$class$", new Class[]{String.class}, null);
		// invoke Class#forName() and return result
		bld.aload(0);
		bld.invoke(Opcode.opc_invokestatic,Class.class,"forName",new Class[]{String.class}, Class.class);
		bld.label(1);
		bld.areturn();
		...
		bld.dup();
		bld.aload(1);
		bld.invoke(Opcode.opc_invokevirtual, ClassNotFoundException.class, "getMessage", new Class[]{}, String.class);
		bld.invoke(Opcode.opc_invokespecial, NoClassDefFoundError.class, "<init>", new Class[]{String.class}, Void.TYPE);
		bld.athrow();
