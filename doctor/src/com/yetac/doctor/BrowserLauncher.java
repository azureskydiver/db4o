 ...
 * <br>Fr�d�ric "Edhil" Hermann (<a href="edhil@neptune.fr">edhil@neptune.fr</A>)
 *
 * @author Eric Albert (<a href="mailto:ejalbert@cs.stanford.edu">ejalbert@cs.stanford.edu</a>)
 *
 * @version 1.3.1 (Released Jul, 1 2000)
 */
public class BrowserLauncher {
	...
	*/
	...
	/** JVM constant for MRJ 2.1 */
	private static final int MRJ_2_1 = 1;

	/** JVM constant for MRJ 2.2 or later */
	...
	*/
	...
	private static Object locateBrowser() {
		...
		switch (jvm) {
			...
			case MRJ_2_2 :
				try {
					// In fact we don't need a browser, just a ref
					// to the com.apple.mrj.MRJFileUtils class
					String name = "openURL";
					Class[] params = new Class[] { name.getClass()};
					Method m = mrjFileUtilsClass.getDeclaredMethod(name, params);
					return m;
				} catch (Exception e) {
					browser = null;
					errorMessage = e.getMessage();
	...
	public static void openURL(String url) throws IOException {
		...
		switch (jvm) {
			...
			case MRJ_2_2 :
				try {
					((Method) browser).invoke(null, new Object[] { url });
				} catch (InvocationTargetException ite) {
					throw new IOException(
						"InvocationTargetException while creating openURL : " + ite.getMessage());
						
					} catch (IllegalAccessException iae) {
					throw new IOException(
						"IllegalAccessException while building openURL : " + iae.getMessage());
